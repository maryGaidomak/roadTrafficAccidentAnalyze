...
Если получаете `ENOENT` или `/app/data/input: No such file or directory`, пересоздайте сервис с новым compose и проверьте mount:

```bash
docker compose -f docker-compose.app.yml up -d --build --force-recreate app
docker compose -f docker-compose.app.yml exec -T app ls -la /app/data
docker compose -f docker-compose.app.yml exec -T app ls -la /app/data/input
```
