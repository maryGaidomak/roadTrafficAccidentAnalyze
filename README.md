...
```bash
docker compose run --rm app npm run import:segments -- ./data/examples/road_segments.sample.json
# или через env:
# docker compose run --rm -e IMPORT_FILE_PATH=./data/examples/road_segments.sample.json app npm run import:segments
```

### 3) Импортировать historical accident stats из файла

```bash
docker compose run --rm app npm run import:historical -- ./data/examples/historical_stats.sample.json
# или через env:
# docker compose run --rm -e IMPORT_FILE_PATH=./data/examples/historical_stats.sample.json app npm run import:historical
```
